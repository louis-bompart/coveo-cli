...
export default class CommerceRecipe extends Command {
   ...
   */
  private async setupDNE(fields: FieldModel[]) {
  ...
  private async additionalCommerceFeatures(fields: FieldModel[]) {
    ...
    // FIXME: here we would need to make the snapshot parameterizable depending on the user's choices
    const field = await this.setupProductGrouping(fields);
    await this.setupDNE(fields);
    return field;
  }
